...
// Simple rate limiting (in-memory for single worker)
...
async function sendAcknowledgmentEmail(data: FormData, env: Env) {
    try {
...
async function sendAdminNotification(data: FormData, env: Env) {
    try {
        await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
                from: '007Phere System <noreply@007phere.com>',
                to: env.ADMIN_EMAIL,
                subject: `New Form Submission - ${data.name}`,
                html: `
          <div style="font-family: Arial, sans-serif; padding: 20px; background: #f5f5f5;">
            <div style="background: white; padding: 30px; border-radius: 8px; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #a78bfa; margin-top: 0;">🔔 New Form Submission</h2>
              <div style="background: #f9f9f9; padding: 15px; border-radius: 6px; margin: 20px 0;">
                <p style="margin: 8px 0;"><strong>Name:</strong> ${sanitizeString(data.name)}</p>
                <p style="margin: 8px 0;"><strong>Email:</strong> ${data.email}</p>
                <p style="margin: 8px 0;"><strong>Phone:</strong> ${data.phone}</p>
                <p style="margin: 8px 0;"><strong>Services:</strong> ${data.selectedServices.join(', ')}</p>
                ${data.message ? `<p style="margin: 8px 0;"><strong>Message:</strong> ${sanitizeString(data.message)}</p>` : ''}
              </div>
              <p style="text-align: center;">
                <a href="https://007phere.com/admin" style="display: inline-block; background: #a78bfa; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">View in Dashboard</a>
              </p>
            </div>
          </div>
        `,
...
export default {
    async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        ...
        try {
            ...
            ctx.waitUntil(
                Promise.all([
                    sendAcknowledgmentEmail(data, env),
                    sendAdminNotification(data, env),
                ])
            );
